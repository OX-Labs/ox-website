...
(function (CoinGeckoApi) {
  CoinGeckoApi['AllCoins'] =
    'coins/markets?vs_currency=usd&page=1&per_page=30&sparkline=false';
...
const CryptoListItem = (props) => {
  ...
  const getClasses = () => {
    const selected =
      state.selectedCrypto && state.selectedCrypto.id === crypto.id;
  ...
  return React.createElement(
    ...
    React.createElement(
      'div',
      { className: 'crypto-list-item-background' },
      React.createElement(
        'h1',
        { className: 'crypto-list-item-symbol' },
        crypto.symbol,
      ),
      React.createElement('img', {
        className: 'crypto-list-item-background-image',
        src: crypto.image,
      }),
    ),
    React.createElement(
      'div',
      { className: 'crypto-list-item-content' },
      React.createElement(
        'h1',
        { className: 'crypto-list-item-rank' },
        crypto.rank,
      ),
      React.createElement('img', {
        className: 'crypto-list-item-image',
        src: crypto.image,
      }),
      React.createElement(
        'div',
        { className: 'crypto-list-item-details' },
        React.createElement(
          'h1',
          { className: 'crypto-list-item-name' },
          crypto.name,
        ),
        React.createElement(
          'h1',
          { className: 'crypto-list-item-price' },
          crypto.price,
        ),
      ),
    ),
  ...
  if (state.status === RequestStatus.Success && state.cryptos.length > 0) {
    const getItems = () => {
      return state.cryptos.map((crypto) =>
        React.createElement(CryptoListItem, { key: crypto.id, crypto: crypto }),
      );
    };
...
const CryptoPriceChart = () => {
  ...
  React.useEffect(() => {
    if (state.chart !== null) {
      const update = () =>
        ChartUtility.update(state.chart, state.points, crypto.change);
      update();
    }
  ...
  const getLoadingSpinner = () => {
    if (state.status === RequestStatus.Loading) {
      return React.createElement(
        'div',
        { id: 'crypto-price-chart-loading-spinner' },
...
const CryptoField = (props) => {
  return React.createElement(
    'div',
    { className: classNames('crypto-field', props.className) },
...
const CryptoDetails = () => {
  ...
  if (crypto !== null) {
    ...
    return React.createElement(
      ...
      {
        id: 'crypto-details',
        className: classNames(sign, { transitioning: state.transitioning }),
      },
      React.createElement(
        'div',
        { id: 'crypto-details-content' },
        React.createElement(
          'div',
          { id: 'crypto-fields' },
          React.createElement(CryptoField, {
            label: 'Rank',
            value: crypto.rank,
          }),
          React.createElement(CryptoField, {
...
const App = () => {
    ...
    const fetch = async () => {
      try {
        ...
        const res = await axios.get(
          `${CoinGeckoApi.Base}/${CoinGeckoApi.AllCoins}`,
        );
        setStateTo(
          Object.assign(Object.assign({}, state), {
  ...
  return React.createElement(
    AppContext.Provider,
    { value: { state, selectCrypto, setStateTo, toggleList } },
    React.createElement(
      'div',
      {
        id: 'app',
